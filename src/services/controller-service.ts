...
export default class ControllerService extends Service {
    ...
    public registerControllers(app: Application): void {
        this.controllers.forEach(controller => {
            ...
            this.container.log.log(`Registered controller ${controller.constructor.name} - "${controller.rootUri}"`, { type: 'endpoints' });
            controller.endpoints.forEach(endpoint => {
                const description = (endpoint.description !== undefined) ? ` (${endpoint.description})` : '';
                this.container.log.log(`    - ${endpoint.method} "${controller.rootUri}${endpoint.uri}"${description}`, { type: 'endpoints' });
            });
        });
    }
}
